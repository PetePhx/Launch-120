...
You may remember our Minilang language from back in the 101 Medium exercises. We
return to that language now, but this time we'll be using OOP. If you need a
refresher, refer back to that exercise.
...
class Minilang
  ...
  def perform(op)
    if op.to_i.to_s == op
      @register = op.to_i
    elsif OPERATIONS.include? op
      send(op.downcase)
    else
      raise RuntimeError, "Invalid token: #{op}"
    end
  end
  ...
  def pop
    @stack.empty? ? raise(RuntimeError, "Empty stack!") : @register = @stack.pop
  end
...
Minilang.new('1 DIV').eval
# Empty Stack!

Minilang.new('0 PUSH 1 DIV PRINT').eval
# divided by 0.

Minilang.new('0 PUSH 1 MOD PRINT').eval
# divided by 0.
